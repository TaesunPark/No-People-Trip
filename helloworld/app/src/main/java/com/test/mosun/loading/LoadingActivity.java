...
//            textView.setText("연결 안됨.");
    ...
    /**
     * 투어리스트 가져옴 > 아직 못함.. 여기 하고 있는 중
     * @param tag
     * @return
     */
    private ArrayList<TourList> getData(String tag)
    {
        SharedPreferences sp = getSharedPreferences("NPT", MODE_PRIVATE);
        String strList = sp.getString(tag, "");

        Gson gson = new GsonBuilder().create();
        Type listType = new TypeToken<ArrayList<TourList>>() {
        }.getType();
    ...
    public void onSaveAreaData() {
        ...
        list.add(new areaItem(0, "area_1", "전주","전주의 ~~"));
        list.add(new areaItem(0, "area_2", "순창","순창의 ~~"));
        AppManager.getInstance().setAreaList(list);
    }

    public void onSaveLocationData(){
        ArrayList<LocationList> list;
        list = new ArrayList<>();
        list.add(new LocationList("어린이대공원 정문", 37.549397, 127.076289));
        list.add(new LocationList("세종관", 37.549964, 127.074527));
        list.add(new LocationList("군자관", 37.549564, 127.073738));
        list.add(new LocationList("대양홀",  37.548611, 127.074499));
        AppManager.getInstance().setLocationList(list);
    }
    ...
    public void onSaveTourListData() {
        ...
        listSeoul.add(new TourList("경복궁", "설명", 37.5792642,126.9778535, kungbokgoungPridictionNumber,0, R.drawable.kgoung));
        listSeoul.add(new TourList("덕수궁", "설명", 37.5657008,126.9740246, ducksugoungPridictionNumber,0, R.drawable.dgoung));
        listSeoul.add(new TourList("창경궁", "설명", 37.5787708,126.9926811, changkunggoungPridictionNumber,0, R.drawable.ckgoung));
        listSeoul.add(new TourList("창덕궁", "설명", 37.5808977,126.9898217, changduckgoungPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("서울스카이", "설명",37.5126108,127.1005055 , seoulSkyPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("아쿠아리움", "설명", 37.513131,127.056094, acuariumPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("서대문형무소역사관", "설명", 37.574271,126.9538823, seodaemunPrisonHistoryMuseumPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("서울대공원", "설명", 37.4277758,127.0088695,seoulParkPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("종묘", "설명", 37.574583,126.9919543, jongmyoPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("국립중앙박물관", "설명", 37.5238879,126.9786427, nationalMuseumPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("서울시립미술관", "설명", 37.5907778,127.0085263, seoulMuseumOfArtPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("세종대왕기념관", "설명", 37.590783,127.0413571, kingSejongPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("남산골한옥마을", "설명", 37.559315,126.9922883, namsangolHanokVillagePridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("선릉", "설명", 37.5029079,127.0168782, sunrungPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("정릉", "설명", 37.4603954,126.9269338, jeongrungPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("헌릉", "설명", 37.4604794,127.0495097, hunrungPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("태릉", "설명", 37.6038265,127.0225271, taerungPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("의릉", "설명", 37.6038317,127.0553579, uirungPridictionNumber,0, R.drawable.cdkoung));
        listSeoul.add(new TourList("영희원", "설명", 37.5885055,127.0414405, younghwiwonPridictionNumber,0, R.drawable.cdkoung));
        //전주
        listJeonju.add(new TourList("경기전", "설명", 35.8153224,127.1476037, gyeonggijeonPridictionNumber,0, R.drawable.cdkoung));
        listJeonju.add(new TourList("국립전주박물관", "설명", 35.8012972,127.0875554, nationalMuseumInjeonjuPridictionNumber,0, R.drawable.cdkoung));
        listJeonju.add(new TourList("스파라쿠아", "설명", 35.8174308,127.1134445, sparacuaPridictionNumber,0, R.drawable.cdkoung));
        listJeonju.add(new TourList("전주 동물원", "설명", 35.8555198,127.1424744, zooInJeonjuPridictionNumber,0, R.drawable.cdkoung));
        listJeonju.add(new TourList("한벽문화관", "설명", 35.8120443,127.1562016, hanbyukPridictionNumber,0, R.drawable.cdkoung));
        listJeonju.add(new TourList("한옥레일바이크", "설명", 35.829531,127.1739387, railBikePridictionNumber,0, R.drawable.cdkoung));
        listJeonju.add(new TourList("한국도로공사수목원", "설명", 35.870964,127.0521402, arboretumPridictionNumber,0, R.drawable.cdkoung));
        //순창
        listSunchang.add(new TourList("강천산", "설명", 35.4087639,127.065336 , kangcheonMountainPridictionNumber,0, R.drawable.cdkoung));
        listSunchang.add(new TourList("황토열매마을", "설명", 35.3192685,127.0533413, fruitVilagePridictionNumber,0, R.drawable.cdkoung));
        listSunchang.add(new TourList("고추장익는마을", "설명", 35.4833446,127.1144903, pepperVilagePridictionNumber,0, R.drawable.cdkoung));
        listSunchang.add(new TourList("장류체험관", "설명", 35.3657696,127.1061715, jangruPridictionNumber,0, R.drawable.cdkoung));
        listSunchang.add(new TourList("산림박물관", "설명", 35.4788907,126.9227723, mountainMuseumPridictionNumber,0, R.drawable.cdkoung));
        listSunchang.add(new TourList("발효소스토굴", "설명", 35.3673458,127.1006084, fermentationSaucePridictionNumber,0, R.drawable.cdkoung));

        Collections.sort(listSeoul, new SortListByDistance());
